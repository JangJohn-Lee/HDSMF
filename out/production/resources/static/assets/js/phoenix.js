...
// 부품 정보 관리 엑셀 다운로드
//const excelFileExportToast = document.getElementById('excel-file-export-toast');
//const excelFileExportToastInstance = new bootstrap.Toast(excelFileExportToast);

//const excelFileExportBtn = document.querySelector('.excel-file-export-btn');
//excelFileExportBtn?.addEventListener('click', () => {
  // 파일 다운로드 시작 토스트 메시지 표시
//  excelFileExportToast.querySelector('.toast-body').innerHTML = `<span class="far fa-arrow-alt-circle-down text-success fs-3 me-3"></span><p class="mb-0 flex-1">파일 다운로드 시작</p>`;
//  excelFileExportToastInstance.show();

//  setTimeout(() => {
    // 엑셀 데이터 생성 및 가공
//    const selectedRows = document.querySelectorAll('[data-bulk-select-row]:checked');
//    const excelData = [];
//    selectedRows.forEach(row => {
//      const rowData = JSON.parse(row.getAttribute('data-bulk-select-row'));
...
//    const excelDataString = JSON.stringify(excelData);
    ...
    // 엑셀 파일 생성 및 다운로드
//    const blob = new Blob([excelDataString], { type: 'application/json' });
//    const url = URL.createObjectURL(blob);

//    const a = document.createElement('a');
//    a.href = url;
//    a.download = 'selected_rows.json'; // 엑셀 파일명 설정
//    document.body.appendChild(a);

    // 토스트 메시지 설정 및 표시
//    excelFileExportToast.querySelector('.toast-body').innerHTML = `<span class="fas fa-check-circle text-success fs-3 me-3"></span><p class="mb-0 flex-1">파일 다운로드 완료</p>`;
//    excelFileExportToastInstance.show();

//    a.click();
//    document.body.removeChild(a);
//    URL.revokeObjectURL(url);
//  }, 2000); // 2초 딜레이 설정 (시작 토스트가 나타나는 시간)
//});
...
const excelFormFileExportBtn = document.querySelector('.excel-form-file-btn');
excelFormFileExportBtn?.addEventListener('click', () => {
  // 파일 다운로드 시작 토스트 메시지 표시
  excelFileExportToast.querySelector('.toast-body').innerHTML = `<span class="far fa-arrow-alt-circle-down text-success fs-3 me-3"></span><p class="mb-0 flex-1">파일 다운로드 시작</p>`;
  excelFileExportToastInstance.show();
